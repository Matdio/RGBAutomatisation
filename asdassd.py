...
def main(n, boolAll):
    ...
    for i in range(n):
        if boolAll == 0:
            nT = Process(target = TransformStretch, args = (resx, resy, data, bitIn, m, lowerClip, higherClip, i, n,))
            Threads.append(nT)
        else:
            nT = Process(target = TransformStretchALL, args = (resx, resy, data, bitIn, m, lowerClip, higherClip, i, n,))
            Threads.append(nT)
        output = []
    for i in range(n):
        toutput.append(0) 
